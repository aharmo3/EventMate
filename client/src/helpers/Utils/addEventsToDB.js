    ...
    for (let i of arrayOfObjects) {
      ...
      if (objectIds.includes(eventID)) {
        ...
        if (eventExists.data.length > 2) {
          ...
          if (i.eventLocation) evLocation = i.eventLocation;
          if (i.currency) evCurrency = i.currency;
          if (i.startingPrice) evStartPrice = i.startingPrice;
          if (i.purchaseLink) pLink = i.purchaseLink;
          if (i.genre) genr = i.genre;
          if (i.subgenre) subg = i.subgenre;
          if (i.eventHost) evHost = i.eventHost;
          if (i.eventType) evType = i.eventType;

          let result = await ClientAPI.addEventDetails(
            userId,
            i.id,
            i.name,
            i.date,
